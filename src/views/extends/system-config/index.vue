  ...
  >
    <el-form ref="formRef" :model="formModel" label-width="110px">
      ...
      <el-form-item label="配置分组">
        <el-input v-model="formModel.configGroup" disabled />
      </el-form-item>
        ...
        <div class="json-form-item__tips">
          <template v-if="dynamicFields.length">
            <div v-for="field in dynamicFields" :key="field.name">
              <div class="json-tip__line">
                <span class="json-tip__name">{{ field.name }}</span>
                <span class="json-tip__type">类型：{{ field.type }}</span>
                <span v-if="field.required" class="json-tip__required">必填</span>
                <span class="json-tip__desc">{{ field.description }}</span>
                <span v-if="formatDefaultValue(field.defaultValue)" class="json-tip__default">
                  默认值：<code>{{ formatDefaultValue(field.defaultValue) }}</code>
                </span>
              </div>
              <div
                v-if="field.type === 'object' && field.children?.length"
                class="json-tip__children"
              >
                <div
                  v-for="child in field.children"
                  :key="`${field.name}-${child.name}`"
                  class="json-tip__child-line"
                >
                  <span class="json-tip__name">{{ field.name }}.{{ child.name }}</span>
                  <span class="json-tip__type">类型：{{ child.type }}</span>
                  <span v-if="child.required" class="json-tip__required">必填</span>
                  <span class="json-tip__desc">{{ child.description }}</span>
                  <span v-if="formatDefaultValue(child.defaultValue)" class="json-tip__default">
                    默认值：<code>{{ formatDefaultValue(child.defaultValue) }}</code>
                  </span>
                </div>
              </div>
            </div>
...
const resetQuery = () => {
  ...
  // 保持默认 integration 分组
  queryParams.configGroup = 'integration'
  handleQuery()
}
...
const resetFormModel = () => {
  ...
  formModel.configGroup = 'integration'
