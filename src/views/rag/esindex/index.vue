<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['rag:es-index:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['rag:es-index:export']"
        >
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EsIndexForm ref="formRef" @success="getList" />
</template>
...
import EsIndexForm from './EsIndexForm.vue'

/** ES索引管理 列表 */
defineOptions({ name: 'EsIndex' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<EsIndexVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EsIndexApi.getEsIndexPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await EsIndexApi.exportEsIndex(queryParams)
    download.excel(data, 'ES索引管理.xls')
  } catch {
  } finally {
