<template>
  <div class="governance-page">
    <ContentWrap>
      <div class="page-head">
        <div>
          <div class="breadcrumb">首页 / AI 数据治理 / <span>基础 AI 定时管理</span></div>
          <div class="page-title">基础增强任务调度</div>
          <div class="page-subtitle">承接现有 ai-schedule：摘要、标签、NER、分类、OCR、向量等基础增强任务统一管理。</div>
        </div>
    ...
    <ContentWrap>
      <template #header>基础 AI 定时任务</template>
      <el-table v-loading="loading" :data="list" stripe>
        ...
        <el-table-column label="操作" fixed="right" min-width="300">
          <template #default="{ row }">
            ...
            <el-button link type="success" @click="handleTrigger(row.id)">运行一次</el-button>
            <el-button link :type="row.status === 1 ? 'warning' : 'primary'" @click="toggleStatus(row)">
              {{ row.status === 1 ? '暂停' : '启用' }}
            </el-button>
            <el-button link type="info" @click="goToTaskLog(row.id)">查看运行日志</el-button>
...
defineOptions({ name: 'AiDataGovernanceBasicSchedule' })
...
const kpis = computed(() => [
  { label: '基础任务', value: total.value, delta: 'ai-schedule' },
  { label: '当前页启用', value: list.value.filter((item) => item.status === 1).length, delta: '可自动执行' },
  { label: '当前页暂停', value: list.value.filter((item) => item.status === 2).length, delta: '已暂停' },
  { label: '任务类型', value: new Set(list.value.map((item) => item.aiTaskType)).size, delta: '增强能力' },
  { label: '绑定模型', value: list.value.filter((item) => item.modelId).length, delta: '模型任务' },
  { label: '治理定位', value: '基础增强', delta: '摘要/标签/NER/OCR' }
])
...
@media (max-width: 768px) { .page-head { align-items: flex-start; flex-direction: column; } .head-actions { width: 100%; flex-wrap: wrap; } .kpi-grid { grid-template-columns: 1fr; } }
</style>
