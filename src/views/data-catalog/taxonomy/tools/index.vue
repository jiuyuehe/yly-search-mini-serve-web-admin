<template>
  <div class="taxonomy-tools-page">
    <ContentWrap>
      <div class="page-head">
        <div>
          <div class="breadcrumb">首页 / 数据目录划分 / <span>主题 Tools / MCP</span></div>
          <div class="page-title">主题标签库 Tools / MCP 发布管理</div>
        </div>
    ...
    <div class="kpi-grid">
      <div class="kpi-card">
        <div class="kpi-label">主题 Tool</div>
        <div class="kpi-value">{{ formatNumber(tools.length) }}</div>
        <div class="kpi-delta">分类分级专属</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-label">MCP 发布</div>
        <div class="kpi-value">{{ formatNumber(publishedCount) }}</div>
        <div class="kpi-delta">平台内状态</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-label">删除类 Tool</div>
        <div class="kpi-value danger">{{ formatNumber(deleteToolCount) }}</div>
        <div class="kpi-delta">需要删除保护</div>
      </div>
      <div class="kpi-card">
        <div class="kpi-label">调用日志</div>
        <div class="kpi-value">{{ formatNumber(logs.length) }}</div>
        <div class="kpi-delta">可追溯</div>
      </div>
    </div>
        ...
        <el-tab-pane label="MCP 发布状态" name="mcp">
          <el-table v-loading="loading" :data="mcpItems" stripe class="dense-table">
            ...
            <el-table-column label="发布状态" width="170">
              <template #default="{ row }">
                <el-tag :type="row.publishStatus === 'PLATFORM_READY' ? 'success' : 'warning'">
            ...
            <el-table-column label="说明" prop="description" min-width="320" show-overflow-tooltip />
          </el-table>
        </el-tab-pane>

        <el-tab-pane label="调用日志" name="logs">
          <el-table v-loading="loading" :data="taxonomyLogs" stripe class="dense-table">
...
import { TagSystemApi, TaxonomyMcpPublishVO } from '@/api/rag/tagsystem'

defineOptions({ name: 'DataCatalogTaxonomyTools' })

const activeTab = ref('tools')
const loading = ref(false)
const tools = ref<GovernanceToolCatalogVO[]>([])
const mcpItems = ref<TaxonomyMcpPublishVO[]>([])
const logs = ref<any[]>([])
const taxonomyToolNames = computed(() => new Set(tools.value.flatMap((item) => [item.name, item.canonicalName])))
const taxonomyLogs = computed(() => logs.value.filter((item) => taxonomyToolNames.value.has(item.toolName)))
const publishedCount = computed(() => mcpItems.value.filter((item) => item.mcpPublished).length)
const deleteToolCount = computed(() => tools.value.filter((item) => item.deleteTool).length)
...
const refresh = async () => {
  ...
  try {
    const [catalog, mcpCatalog, logData] = await Promise.all([
      ...
      TagSystemApi.getTaxonomyMcpPublishCatalog(),
      GovernanceApi.listToolLogs()
    ])
    tools.value = (catalog || []).filter((item) => item.module === '主题分类分级')
...
<style scoped>
.taxonomy-tools-page { display: flex; flex-direction: column; gap: 12px; }
